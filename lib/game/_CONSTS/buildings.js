module.exports = {
  ...
  warehouse: {
    name: "倉庫",
    description: "增加資源容量上限",
    maxLevel: 10,
    baseCost: { gold: 300 },
    upgradeCost: (level) => ({ gold: level * 150 }),
    effect: (level) => ({ materialCapacityBonus: 100 * level }),
  },
  guildHall: {
    name: "公會大廳",
    description: "解鎖公會系統",
    maxLevel: 10,
    baseCost: { gold: 500, wood: 100, iron: 50 },
    upgradeCost: (level) => ({ gold: level * 200, wood: level * 60, iron: level * 30 }),
  },
  lumberMill: {
    name: "木材廠",
  ...
  archery: {
    name: "弓箭塔",
    description: "訓練弓箭單位自動巡邏",
    maxLevel: 10,
    baseCost: { gold: 350, wood: 80 },
    upgradeCost: (level) => ({ gold: level * 140, wood: level * 50 }),
    effect: (level) => ({ unitCapacityBonus: 4 * level }),
  },
};
