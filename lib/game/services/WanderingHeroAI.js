...
class WanderingHeroAI {
  ...
  async processExploring(hero, user, result) {
    ...
    if (combatResult.victory) {
      ...
      // Add battle log for wandering hero combat
      user.addBattleLog({
        category: teamHeroes.length > 1 ? "team_combat" : "solo_combat",
        zone: hero.currentZone,
        difficulty: hero.currentSubZone,
        heroNames: teamHeroes.map(h => h.name),
        victory: true,
        goldReward: combatResult.goldReward,
        xpGained: combatResult.xpGained,
        magicStonesFound: combatResult.magicStonesFound,
        logMessages: combatResult.logMessages,
      });
    } else {
